...
Three waves are *gating* — their entry requires an extra invariant:

- **W24 Stub Clearance Gate**: Active stubs table empty before Stage 2
  self-hosting begins.
- **W25 Stage 2 and Self-Hosting**: Phase 00 re-verifies the Active
  stubs table is still empty.
- **W26 Native Backend Transition**: Phase 00 re-verifies the Active
  stubs table is still empty.
...
| Project | Active stubs table empty before self-hosting; perf gate passes before retiring Go/C |
